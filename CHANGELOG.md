...
### Fixed
- **OpenCode Zen y Cloudflare Workers AI: bloqueo CORS en el navegador.** opencode.ai y api.cloudflare.com no envían `Access-Control-Allow-Origin` → las llamadas directas desde el frontend en Cloud Run se bloquean con `Failed to fetch`. Añadidos proxies backend `/api/openzen` y `/api/cloudflare` en `server/index.js` (mismo patrón que Gemini y NIM). El frontend ahora apunta a `/api/openzen` y `/api/cloudflare`; el servidor reenvía al upstream con la key en memoria, sin persistir ni loguear (Zero-Storage).
...
### Changed
- `server/index.js`: endpoints proxy `POST /api/openzen` y `POST /api/cloudflare` con rate limiters propios (40 req/min) y helper `pipeUpstream()` para streaming SSE; el proxy de Cloudflare construye la URL del upstream con `accountId` recibido del body.
...
- `client/src/i18n/es.ts` y `en.ts`: `provider.openzen.*` y `provider.cloudflare.*` actualizados (sin mención a token "public"; Cloudflare usa catálogo estático ZCode).
- `client/src/services/__tests__/providers.test.ts`: tests adaptados a proxies y catálogos estáticos.
- `server/index.js`: logs upstream con status/content-type para debugging de errores 401/403/404/429.
