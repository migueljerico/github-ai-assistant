...
### Changed
- **NVIDIA NIM: acceso directo eliminando el proxy backend.** `nvidia.chatEndpoint` cambiado de `/api/nim` (proxy backend) a `https://integrate.api.nvidia.com/v1/chat/completions` (acceso directo), siguiendo el patrón de Zenmux y otros proveedores OpenAI-compatible. La lista hardcodeada `NIM_FALLBACK` (12 modelos curados) se mantiene intacta. Si NIM sigue sin enviar CORS en producción, el proxy `/api/nim` permanece en `server/index.js` como fallback.

### Notes
- Cambio de código por ZCode (mimo-v2.5-free). 531/531 tests de cliente verdes, build limpio. `server/index.js` no se tocó; el proxy `/api/nim` queda disponible por si se necesita revertir.
...
### Fixed
- **Lint: restaurada prop `onClearAllFiles` en `ChatInputProps`.** Era requerida por `App.tsx` y `ChatInputStop.test.tsx` (interfaz compartida). Se marca con `// eslint-disable-line` para evitar el warning de unused-vars sin romper el tipado del consumidor.
