...
Registro histórico de todas las características, optimizaciones y correcciones resueltas en el proyecto **Asistente de IA de GitHub**, ordenadas cronológicamente desde la versión v1.0.0 hasta la v4.0.0.
...
| - | Menú progresivo de herramientas avanzadas del chat (`ChatToolsMenu`) para reducir carga visual (UI Refactor). | client/src/components/chat/{ChatToolsMenu.tsx, ChatInput.tsx} | v4.0.5 |
| - | Fix QwenCloud (400 Bad Request Payload Too Large): Filtrar data URIs de base64 de imágenes adjuntas en `truncateByLines` para evitar que revienten el prompt al pedir documentar archivos o repositorios. | client/src/services/gemini.ts | v4.0.5 |
