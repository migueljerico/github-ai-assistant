...
| v4.0.43 | Error 503 (Model Overloaded) en modelos recién lanzados (Gemini 3.8 Flash) al documentar, y reintentos congelados | 1) Modelos nuevos reciben picos masivos de demanda en Google AI Studio; la doc completa enviaba peticiones al endpoint saturado. El modo ligero simplemente mandaba menos tokens al mismo endpoint, repitiendo el 503. 2) En `DocumentFlowModal`, `handleGenerateRepo` condicionaba el reseteo del error a no ser modo ligero (`if (!opts?.lightMode) setFlowError(null)`), lo que congelaba la UI en *"Generando documentación..."* con el banner de error permanente. | 1) Fallback automático resiliente: ante 503 por sobrecarga en `gemini`, conmutar automáticamente a `gemini-2.5-flash` (misma API key, alta disponibilidad) y notificar en chat. 2) Resetear siempre incondicionalmente `flowError` al iniciar cualquier intento. 3) Ofrecer controles directos en UI (botón de conmutación a Gemini 2.5 Flash y selector de modelos en modal) para evitar callejones sin salida. |
| v4.0.44 | "⚠️ Tiempo de espera agotado al generar la documentación completa" SIEMPRE al documentar un repo muy grande con un modelo de razonamiento (caso real: `estudio-360-smart-learn-netlify` + Qwen 3.8 Max 0902 vía QwenCloud), incluso subiendo el timeout en ⚙️ | **Doble techo de 300s:** (1) el default del modo completo en `generateRepoDocs` era 300s — un modelo de razonamiento supera los 5 min por llamada con repos enormes (120 archivos × 80 líneas + salida de 8.192 tokens); (2) el request timeout de Cloud Run es 300s por defecto (ni `deploy.sh` ni el trigger de CD pasaban `--timeout`), así que aunque ⚙️ se subiera a 600s la plataforma cortaba con 504 a los ~300s, que el cliente clasificaba como el mismo `'timeout'`. Callejón sin salida invisible desde la UI. | Al subir un timeout, alinear **todas las capas de la cadena**: cliente → proxy (tope) → plataforma. v4.0.44: default del modo completo a 600s (= tope del proxy = máximo de ⚙️), `--timeout 600` en `deploy.sh` y `gcloud run services update --timeout=600` una vez al servicio vivo (persiste entre deploys del CD porque el trigger no especifica el flag). Arreglo sostenible a largo plazo: streaming con timeout por inactividad y contexto adaptativo para modelos lentos (#77 en roadmap). Tests de regresión: `gemini.test.ts → 'generateRepoDocs - timeout por defecto según modo (v4.0.44)'`. |
| v4.0.44 | Repos muy grandes nunca se documentaban en modo completo con modelos de razonamiento (caso real: `estudio-360-smart-learn-netlify` + Qwen 3.8 Max 0902 vía QwenCloud): siempre "⚠️ Tiempo de espera agotado", incluso subiendo el timeout en ⚙️ | **Doble techo de 300s**: el default del modo completo en `generateRepoDocs` (`config?.timeoutMs ?? 300_000`) y el request timeout de la plataforma Cloud Run (default 300s; ni `deploy.sh` ni el trigger de CD pasaban `--timeout`). Subir ⚙️ a 600s no servía: la plataforma cortaba con 504 a los ~300s y el cliente lo clasificaba como el mismo `'timeout'`. Callejón sin salida invisible para el usuario. | Al subir un timeout, verificar **la cadena completa**: cliente → proxy (`getUpstreamTimeout`) → plataforma. Alinear el default con el tope del proxy y el máximo de ⚙️ (600s), incluir `--timeout 600` en `deploy.sh` y aplicar `gcloud run services update --timeout=600` una vez al servicio vivo (persiste entre deploys del CD). Tests de regresión: `gemini.test.ts → 'generateRepoDocs - timeout por defecto según modo (v4.0.44)'`. Arreglo sostenible a largo plazo registrado como #77 (streaming con timeout por inactividad). |
...
| **Antigravity 2.0 / Gemini 3.8 Flash** | Google (Antigravity 2.0) | **v4.0.43:** Fallback automático resiliente ante error 503 (sobrecarga de servidor) al documentar repositorios con Gemini hacia `gemini-2.5-flash`, exposición de `setModel` en `AIProviderContext`, indicador de modelo activo, botón de cambio directo y selector de modelo en banner de sobrecarga en `DocumentFlowModal`, reseteo incondicional de error al reintentar y sincronización en 13 idiomas. +10 tests unitarios (1.360 tests en verde: 1.300 cliente + 60 servidor, 0 fallos), lint 0 errores y build limpia. | Septiembre 2026 |
| **ZCode (Qwen 3.8 Max 0902)** | Qwen 3.8 Max 0902 (ZCode) | **v4.0.44:** Diagnóstico y fix del doble techo de 300s (default del cliente en `generateRepoDocs` + request timeout de Cloud Run) que impedía documentar repos muy grandes con modelos de razonamiento — caso real reportado por el autor: `estudio-360-smart-learn-netlify` vía QwenCloud. Default del modo completo 300s→600s alineado con el tope del proxy y el máximo de ⚙️, `--timeout 600` en `deploy.sh`, documentación del `gcloud run services update` en MANUAL_TECNICO, trampa nueva en CLAUDE.md §5, lección en §4, roadmap **#77** (streaming con timeout por inactividad) y +3 tests de regresión (1.363 tests en verde: 1.303 cliente + 60 servidor), lint 0 errores y build limpia. | Septiembre 2026 |
| **ZCode (builtin:zai-start-plan/GLM-5.3-Flash)** | GLM-5.3-Flash (ZCode / Zhipu) | **v4.0.48:** fix doble — (1) preservación de la vista previa al actualizar la documentación: `fetchRepoTreeRecursive` expone `binaryPaths` (blobs binarios del árbol completo —imágenes incluidas— sin contenido), `runDocumentRepo` los propaga vía `docOptions` y `generateRepoDocs` lista "IMÁGENES Y ARCHIVOS BINARIOS PRESENTES EN EL REPOSITORIO" y añade la REGLA CRÍTICA DE VISTA PREVIA (solo eliminar referencias de imagen confirmadas como huérfanas; preservar intactas las galerías cuyas imágenes existen) — la directriz anti-huérfanos anterior solo veía los blobs de texto y borraba sistemáticamente las capturas publicadas; (2) modelos de razonamiento sin salida: modo completo 8192→16384 tokens (el think interno consume `max_tokens`) y error pedagógico específico cuando content llega vacío tras razonar (`reasoning_content` poblado o `finish_reason: 'length'`, streaming y no-streaming). +10 tests (1.403 en verde: 1.335 cliente + 68 servidor), lint 0 errores y build limpia. | Septiembre 2026 |
| **ZCode (builtin:zai-start-plan/GLM-5.3-Flash)** | GLM-5.3-Flash (ZCode / Zhipu) | **v4.0.47:** Auditoría integral de los 11 proveedores de IA con la metodología del fix de OpenCode (v4.0.45/46): verificación en vivo de los endpoints de chat/modelos de todos los proveedores (401/403 sin key = vivos) y de los 9 proxies de `server/index.js`, y reconstrucción de los fallbacks desactualizados — 4 `defaultModel` muertos corregidos (OpenRouter `gpt-oss-20b:free` → `google/gemma-4-26b-a4b-it:free`, Zenmux `deepseek-v4-flash-free` → `sapiens-ai/agnes-2.5-flash`, Kilo `ling-3.0-flash:free` → `ling-3.0-flash-fin:free`, BazaarLink `deepseek-v4-flash:free` → `qwen/qwen3.7-flash:free`) y 13 ids retirados limpiados de NIM/Ollama/OpenZen. `MODEL_LABELS` sincronizado. +1 test de regresión de auditoría (1.393 tests en verde: 1.325 cliente + 68 servidor), cobertura global 97,86% statements, lint 0 errores y build limpia. | Septiembre 2026 |
| **ZCode (deepseek-v4-pro-0813)** | DeepSeek (vía ZCode) | **Fix #132 (2026-09-07):** adaptación de `@testing-library/jest-dom` 7.x a vitest 5 (Dependabot PR #132, `@vitest/coverage-v8` 4.1.11 → 5.0.0). Vitest 5 cambió la firma de `interface Assertion<T>` a `interface Assertion<R extends void \| Promise<void>, T = unknown>`, y la aumentación oficial de jest-dom 7.x no se fusiona con la nueva firma — `tsc -b` reventaba con TS2339 en cada `toBeInTheDocument`/`toHaveAttribute`/`toHaveTextContent` de los 1.335 tests cliente (rompía CI 'E2E' y bloqueaba el release). Solución: `client/src/test/jest-dom.d.ts` re-declara la aumentación sobre la nueva firma (`extends TestingLibraryMatchers<unknown, R>`) y `client/src/test/__tests__/jestDomTypes.test.tsx` añade test de regresión. Sin bump de versión (fix de tipos de dependencias, no de comportamiento). Verificado: tsc -b 0 errores, lint 0 errores, 1.336/1.336 tests, cobertura 97,87% lines, build limpia, server 68/68. CI del PR y de `main`: test, e2e, server-test, security, codecov/patch y CodeQL en verde. CD a Cloud Run automático. | Septiembre 2026 |
| **ZCode (Kimi K3)** | Moonshot AI (vía ZCode) | **Revisión cruzada del fix #132 (2026-09-07):** a petición del autor, **revisión forense del handoff de v4.0.48** (que erróneamente mencionaba "⏳ NO se desplegó a producción: `npm run deploy`"). Constató que la documentación canónica (`AGENTS.md` §1.10, `CLAUDE.md` §8, `METODOLOGIA_IA.md` §2 paso 9) deja explícito que el deploy a Cloud Run **es automático** vía trigger de Cloud Build en cada push a `main` — y que el propio handoff de v4.0.48 ya citaba los runs de `E2E / main / push` en verde como evidencia del CD. Diagnóstico: la frase del handoff de v4.0.48 era **ruido residual heredado de un asistente previo** (probablemente Kimi K2.7, autor de v3.32.4), no una acción pendiente. Kimi K3 también verificó que el fix de tipos del #132 era correcto y que la ausencia de bump de versión era deliberada (parche de deps, no cambio de comportamiento), evitando así un bump innecesario. | Septiembre 2026 |
