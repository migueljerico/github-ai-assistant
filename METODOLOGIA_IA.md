...
| **ZCode (step-3.7-flash-free)** | GLM-5.2 (ZCode) | **v3.30.0:** Fix crash multi-archivo en flujo de documentación (#57 fix). El `DocumentFlowModal` recibía solo `fileContext[0]` y perdía el resto, causando pantalla de ErrorBoundary. Ahora recibe `allAttachedFiles`, muestra primary/extras en Paso 2, auto-puebla extras en Paso 4 y fusiona no-principales en crear+documentar. 504 tests verdes, build limpio, lint 0 errores. | Julio 2026 |
